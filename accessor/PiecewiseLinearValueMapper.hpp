...
class PiecewiseLinearValueMapper : public ValueMapper,
                                   public Configurable<PiecewiseLinearValueMapperConfig<N>> {
   public:
    /**
     *	@brief	开始校准.
...
float PiecewiseLinearValueMapper<N>::value_get(uint32_t raw_value) {
    ...
    for (size_t i = 0; i < N - 1; ++i) {
        if (wrap_value >= this->config.in_points[i] &&
            wrap_value <= this->config.in_points[i + 1]) {
#if PIECEWISE_LINEAR_VALUE_MAPPER_ENABLE_K
            _last_value = this->config.out_points[i] +
                          this->config.out_k[i] * (wrap_value - this->config.in_points[i]);
#else
            _last_value = this->config.out_points[i] +
                          (this->config.out_points[i + 1] - this->config.out_points[i]) /
                              (this->config.in_points[i + 1] - this->config.in_points[i]) *
