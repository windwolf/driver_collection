...
Result Command::_do_send(CommandFramePhase phase, void *data, uint32_t size, DataWidth dataWidth,
                         bool isWrite, WaitHandler &waitHandler, uint32_t scope)
{
    auto rst = media_operate(phase, data, size, dataWidth, isWrite, waitHandler);
    if (rst != Result_OK)
    {
        return rst;
    }
...
Result Command::send(CommandFrame &frame, WaitHandler &waitHandler)
{
    ...
    do
    {
        ...
        {
            rst = _do_send(CommandFramePhase_Command, &frame.commandId, 1, DATAWIDTH_8, true,
                           waitHandler, scope);
            if (rst != Result_OK)
            {
                break;
        ...
        {
            rst = _do_send(CommandFramePhase_Address, &frame.address, 1, frame.addressBits, true,
                           waitHandler, scope);
            if (rst != Result_OK)
            {
                break;
        ...
        {
            rst = _do_send(CommandFramePhase_AltData, &frame.altData, 1, frame.altDataBits, true,
                           waitHandler, scope);
            if (rst != Result_OK)
            {
                break;
        ...
        {
            rst = _do_send(CommandFramePhase_DummyCycle, nullptr, frame.dummyCycles, DATAWIDTH_8,
        ...
        {
            rst = _do_send(CommandFramePhase_AltData, frame.data, frame.dataSize, frame.dataBits,
                           frame.isWrite, waitHandler, scope);
            if (rst != Result_OK)
            {
                break;
            }
