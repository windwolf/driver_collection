...
OP_RESULT pin_device_set(PinDevice *device, PIN_DEVICE_STATUS parent) {
    HAL_GPIO_WritePin((GPIO_TypeDef *)device->base.instance, device->pinMask,
                      parent ^ device->inverse);
    return OP_RESULT_OK;
};
...
OP_RESULT pin_device_get(PinDevice *device, PIN_DEVICE_STATUS *parent) {
    ...
    rst =
        (PIN_DEVICE_STATUS)HAL_GPIO_ReadPin((GPIO_TypeDef *)device->base.instance, device->pinMask);
...
OP_RESULT pin_device_mode_set(PinDevice *device, PIN_DEVICE_MODE mode) {
    LL_GPIO_SetPinMode((GPIO_TypeDef *)device->base.instance, device->pinMask,
                       (mode == PIN_DEVICE_MODE_INPUT) ? LL_GPIO_MODE_INPUT : LL_GPIO_MODE_OUTPUT);
    return OP_RESULT_OK;
};
