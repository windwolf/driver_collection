...
OP_RESULT sd_device_block_create(SdDevice *device, Block *block, Buffer8 buffer) {
    return block_create(block, device, device->blockSize, device->blockSize, device->blockSize,
                        false, BLOCK_MODE_BLOCK, BLOCK_MODE_BLOCK, BLOCK_MODE_BLOCK, buffer,
                        (BLOCK_READ_WRITE_CB)&sd_device_read, (BLOCK_READ_WRITE_CB)&sd_device_write,
                        (BLOCK_ERROR_CB)&sd_device_erase);
}
