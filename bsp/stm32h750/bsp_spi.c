...
static uint32_t bits_switch(SPI_HandleTypeDef *handle, DeviceDataWidth width, uint32_t size) {
    uint32_t stream_number_tx =
        (((uint32_t)((uint32_t *)handle->hdmatx->Instance) & 0xFFU) - 0x010UL) / 0x018UL;
    uint32_t dma_base_tx =
        (uint32_t)((uint32_t *)handle->hdmatx->Instance) - stream_number_tx * 0x018UL - 0x010UL;
    uint32_t stream_number_rx =
        (((uint32_t)((uint32_t *)handle->hdmarx->Instance) & 0xFFU) - 0x010UL) / 0x018UL;
    uint32_t dma_base_rx =
        (uint32_t)((uint32_t *)handle->hdmarx->Instance) - stream_number_rx * 0x018UL - 0x010UL;
...
OP_RESULT spi_device_rx(SpiDevice *device, void *data, uint32_t size, DeviceDataWidth width,
                        uint8_t dummyCycleCount) {
    SPI_HandleTypeDef *handle   = (SPI_HandleTypeDef *)(device->base.instance);
    uint32_t           byteSize = bits_switch(handle, width, size);
    if (size < 0) {
        return OP_RESULT_GENERAL_ERROR;
