...
enum ST7735_CMD
{
    ...
    ST7735_CMD_READ_DISPLAY_PIXEL_FORMAT =
        0x0CU, /* Read Display Pixel: RDDCOLMOD               */
    ST7735_CMD_READ_DISPLAY_IMAGE_MODE = 0x0DU, /* Read Display Image: RDDIM */
    ST7735_CMD_READ_DISPLAY_SIGNAL_MODE =
        0x0EU, /* Read Display Signal: RDDSM                  */
    ...
    ST7735_CMD_DISPLAY_INVERSION_OFF =
        0x20U, /* Display inversion off: INVOFF               */
    ...
    ST7735_CMD_TEARING_EFFECT_LINE_OFF =
        0x34U, /* Tearing effect line off: TEOFF              */
    ST7735_CMD_TEARING_EFFECT_LINE_ON =
        0x35U, /* Tearing effect mode set & on: TEON          */
    ST7735_CMD_MEMORY_DATA_ACCESS_CONTROL =
        0x36U, /* Memory data access control: MADCTL          */
    ST7735_CMD_IDLE_MODE_OFF = 0x38U, /* Idle mode off: IDMOFF */
    ST7735_CMD_IDLE_MODE_ON = 0x39U,  /* Idle mode on: IDMON  */
    ST7735_CMD_INTERFACE_PIXEL_FORMAT =
        0x3AU, /* Interface pixel format: COLMOD              */
    ST7735_CMD_FRAME_RATE_CTRL1 =
        0xB1U, /* In normal mode (Full colors): FRMCTR1       */
    ST7735_CMD_FRAME_RATE_CTRL2 = 0xB2U, /* In Idle mode (8-colors): FRMCTR2 */
    ST7735_CMD_FRAME_RATE_CTRL3 =
        0xB3U, /* In partial mode + Full colors: FRMCTR3      */
    ST7735_CMD_FRAME_INVERSION_CTRL =
        0xB4U, /* Display inversion control: INVCTR           */
...
class ST7735 : public ST77xx
{
  public:
    ...
    Result display_window_set(uint16_t x1, uint16_t y1, uint16_t x2,
                              uint16_t y2);
    ...
    Result rect_draw(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                     uint16_t *data);
    Result rect_fill(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                     uint16_t color);
