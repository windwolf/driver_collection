...
DEVICE_STATUS w25qxx_spi_create(W25QXX_SPI *instance, Buffer buffer, FiveStepCommandCient *command)
{
    ...
    command->flagBits.addressBits = DEVICE_DATAWIDTH_24;
...
{
    FiveStepCommandCient *cmd = instance->command;
    ...
    five_step_command_client_send(cmd);
    five_step_command_client_cplt_wait(cmd, TX_WAIT_FOREVER);
